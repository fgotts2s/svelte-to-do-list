...
const eventHandler = () => {
  ...
  const pathParts = window.location.pathname.replace('/svelte-to-do-list', '').split('/');
...
window.addEventListener('popstate', () => {
  // Garbage collection: Trigger onDestroy to remove event handlers from window object.
  const nodeSvelteToDoLists = document.querySelector('svelte-to-do-lists');
  if (nodeSvelteToDoLists) {
    nodeSvelteToDoLists.$destroy();
  }
  const nodeSvelteToDoList = document.querySelector('svelte-to-do-list');
  if (nodeSvelteToDoList) {
    nodeSvelteToDoList.$destroy();
  }
  // Execute the previously defined event handler.
  eventHandler();
});